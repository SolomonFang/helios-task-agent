...
飞书任务中心 / 文档
  ...
  → 「写进 helios-kanban」：展示标题 + 需求摘要草稿 → 系统闸门确认（卡片带标题+描述预览）→ create（不自动 start）
...
| 机制 | 说明 |
|------|------|
| 写操作闸门 | 建/改/删任务、start/stop/follow-up、审批、飞书发消息等写操作，执行前必须经你确认：终端 y（仅此次）/b（同类免问）/N，飞书端弹三按钮确认卡片（或回复「确认/都允许/取消」；普通操作 120 秒、破坏性操作 300 秒超时自动拒绝；决策/超时后卡片原地更新为终态，按钮失效） |
| 批量确认 | 选「同类免问」后 10 分钟内同类型写操作自动放行（批量建任务不用点 10 次）；「确认」默认**仅此次**；删除/取消/停止/deny 等破坏性操作始终逐次确认；免问期间回复「恢复确认」（终端 `/confirm on`）立即撤销 |
...
bot 模式下每 60s 轮询看板：任务进入待审阅（inreview，附 diff 直链）/完成（附结果摘要）/取消、执行失败、新待审批（附标题列表）→ 主动推送飞书（带看板链接与下一步行动提示），并同时注入会话上下文——收到推送后直接回「这个怎么样 / 帮我 review」即可。首轮只建基线，快照存 `watch-state.json`，重启不重复打扰。`KANBAN_WATCH=0` 关闭，`KANBAN_WATCH_INTERVAL_SEC` 调间隔。

## MCP 健康监督（bot）

MCP 连接由 supervisor 每 60s 探测，掉线自动降级 `hk_cli` 并重连，恢复后自动切回（掉线与恢复都会推送通知）。
...
- 终端：`/help` `/config` `/status` `/tools` `/memory` `/clear` `/confirm on`（撤销「同类免问」）`/exit`（任务运行中按 Ctrl+C 只中断不退出）
- 飞书 bot：`/help` `/status` `/tools` `/memory` `/clear` `/confirm` `/stop`（中断当前任务，待确认写操作一并取消）；回复「恢复确认」撤销「同类免问」
- 「以后都从这个飞书地址同步任务：\<链接\>」
...
bot 支持文字与富文本（post）消息；长回复自动拆分多条；处理期间原地更新进度（「⏳ 处理中…（调用工具 xxx）」）；任务繁忙或确认卡片挂起时，新消息会收到排队/提示回执；确认卡片在决策/超时后原地更新为终态（按钮失效）。
