...
bot 模式下每 60s 轮询看板：任务完成（附结果摘要）/取消、执行失败、新待审批（附标题列表）→ 主动推送飞书（带看板链接与下一步行动提示），并同时注入会话上下文——收到推送后直接回「这个怎么样 / 帮我 review」即可。首轮只建基线，快照存 `watch-state.json`，重启不重复打扰。`KANBAN_WATCH=0` 关闭，`KANBAN_WATCH_INTERVAL_SEC` 调间隔。
