...
| 机制 | 说明 |
|------|------|
| 写操作闸门 | 建/改/删任务、start/stop/follow-up、审批、飞书发消息等写操作，执行前必须经你确认：终端 y（仅此次）/b（同类免问）/N，飞书端弹三按钮确认卡片（或回复「确认/都允许/取消」；普通操作 120 秒、破坏性操作 300 秒超时自动拒绝） |
| 批量确认 | 选「同类免问」后 10 分钟内同类型写操作自动放行（批量建任务不用点 10 次）；「确认」默认**仅此次**；删除/取消/停止/deny 等破坏性操作始终逐次确认 |
...
- 终端：`/help` `/config` `/status` `/tools` `/memory` `/clear` `/exit`（任务运行中按 Ctrl+C 只中断不退出）
