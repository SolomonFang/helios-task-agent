...
- AI 审查结果改为中文 HTML 报告链接：`--background` 注入简体中文输出要求；完整结果渲染为自包含 HTML（轻量 markdown：标题/列表/代码围栏/行内格式，全量转义防注入），写入数据目录 `reviews/`（30 天自动清理），由 bot 内置静态服务托管（随机空闲端口、仅服务目录内 `*.html`、路径穿越 404，链接主机名取 `HELIOS_KANBAN_URL`），飞书只推一条报告链接，长结果不再按 4000 字符截断；报告服务启动失败时回退原文本推送
