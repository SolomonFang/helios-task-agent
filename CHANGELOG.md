...
- 配置向导首步（配置模型）支持跳过：已配置过时（`/config` 重配、`bot --reconfig`）预设列表末尾追加「跳过，保留当前模型（xxx）」项——TTY 箭头列表直接选中（数字键 7 直选），非 TTY 编号列表输入 `0` 或直接回车（默认值由 1 改为 0 = 跳过）；选中后保留现有 Base URL / API Key / 模型名，不再追问 API Key、不重复联网校验，直接进入看板默认值等后续步骤；首次配置（`isConfigured()` 为假）不出现该项，行为不变（`src/config/config-wizard.ts`）
