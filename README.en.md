...
| Batch approval | “Allow similar” for 10 minutes on the same write class; plain confirm is **once**. Destructive ops (delete/cancel/stop/approve/start/archive/merge/push/execute) always re-ask. **Feishu message writes (lark sends) never batch**; kanban writes do offer the “allow-similar 10 min” button on the confirm card. `/confirm revoke` or 恢复确认 revokes (`/confirm on` kept as a legacy alias) |
